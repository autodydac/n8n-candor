...
- **A real gap in that first scan, found and fixed, disclosed rather than quietly corrected.** The first sanitization pass over `/workflows` targeted credential references and the builder's name, but missed an internal Docker container file path (`/home/node/.candor-files/...`) present in several node parameters — an internal-infrastructure detail this same page already commits to excluding. A post-push scan of the *actually-pushed* repository (the same discipline described above) caught it; it was genericized and re-verified clean in an immediate follow-up commit. Named here because a security page that only describes successful scans, and never a scan that caught something, is less credible than one that shows the process actually working.
