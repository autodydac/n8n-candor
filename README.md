...
- **Real concurrency testing, not just sequential — and pushed further in a follow-up pass.** 86 real form submissions across 3 batches (up to 50 fully concurrent) proved the dedup gate and append-only write path hold under real load: zero corruption, zero circuit-breaker trips, zero failures. A separate 15-way concurrent test proved the human-approval gateway's session mechanism and append-only decision log hold too — and surfaced a real n8n platform-behavior finding (a single held-open connection doesn't reliably receive a multi-page form's async result; short polling does), later confirmed in a real browser to be specific to that test client, not a real platform limitation. A follow-up pass pushed to 60 concurrent (zero data loss, but a real, honestly-reported latency ceiling — per-execution time spread 10.7s–49.1s within one burst) and separately proved a circuit breaker's complete open→cooldown→recover lifecycle end to end. Full results in [`docs/LOAD-AND-CONCURRENCY-TESTING.md`](docs/LOAD-AND-CONCURRENCY-TESTING.md).
...
- **Cost governance, including a real fix for a real inefficiency found in testing.** Claude Haiku handles high-volume extraction; Claude Sonnet is reserved for the one step (recruiter briefing) where phrasing nuance justifies the cost. A real architectural inefficiency (the job posting was being re-extracted on every submission, not cached) was found, quantified (43% of one load test's spend), and then actually fixed — a content-hash cache now short-circuits repeat extractions of an unchanged posting, reconfirmed live and still active in a later pass. Real measured cost with the cache in place: ≈$0.0053/candidate for intake+scoring on repeat postings, ≈$0.008/candidate for a briefing.
...
- **Self-audited, not just self-reported.** A later pass went back over this project's own prior claims and found real gaps: a stale doc entry (a fix that had already shipped but still read as "deferred"), a real sanitization miss in this repo's own first public push (an internal file path, caught by a post-push scan and fixed), and a record-count discrepancy that was tracked down to its exact real cause rather than left as a rounding note. All reported and fixed the same way everything else here is — with real evidence, not narrated success.
...
- ~~JD re-extraction runs on every submission~~ **Fixed.** A real, quantified inefficiency (43% of one load test's spend) found in testing, then closed with a content-hash cache — repeat submissions against an unchanged posting now skip the extraction call entirely, reconfirmed live and still active as of the most recent pass. The workflow export in `/workflows/CANDOR-04.json` includes the real cache nodes.
